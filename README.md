[README.md](https://github.com/user-attachments/files/29683556/README.md)
...
- SVG upload support by converting SVG uploads to PNG before AI analysis
...
6. If application data is provided, the app compares extracted values against that data.
...
- `sample-application-data.json` with the corresponding expected application values for all 10 products.
...
SVG sample files are supported by the web app. They are converted in the browser to PNG before being sent to the server-side AI analysis route, because the OpenAI image endpoint accepts raster image formats such as PNG/JPEG/WebP rather than SVG.
