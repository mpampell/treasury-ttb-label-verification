...
- **Application data import**: CSV upload in the browser loads structured expected application rows keyed by label file name.
...
5. If application data is present from manual entry or CSV import, validation logic compares extracted values to that data.
...
The app targets under-five-second results for simple labels. The browser resizes uploaded images to a 1200px maximum edge and sends compressed JPEG data to reduce upload and model latency. Actual speed depends on image quality, network latency, model latency, and batch size. Batch processing is sequential in this version to keep rate-limit behavior predictable.

## Trade-offs

- Image preprocessing is limited to browser display and model-side vision interpretation.
- The app strictly validates warning wording and uppercase lead-in. It asks the vision model to note visible font-weight, size, cropping, and placement concerns, but final visual formatting determinations remain subject to human review.
