...
function validateWarning(expected: string, extracted: string) {
  ...
  const normalizedExtracted = normalizeValue(extracted);
  const containsCore =
    normalizedExtracted.includes("surgeon general") &&
    normalizedExtracted.includes("pregnancy") &&
    normalizedExtracted.includes("drive a car") &&
    normalizedExtracted.includes("health problems");
...
export function compareLabel(
  ...
  const hasApplicationData = Object.keys(FIELD_LABELS).some((key) => {
    const fieldKey = key as FieldKey;
    return Boolean(application[fieldKey]?.trim());
  });
  ...
  const fields = Object.keys(FIELD_LABELS).map((key) => {
    ...
    if (!expected && OPTIONAL_FIELDS.has(fieldKey)) {
      return {
        ...
        expected,
        extracted: actual,
        status: actual ? ("extracted" as const) : ("match" as const),
        confidence: extractedField?.confidence ?? 0,
        explanation: actual
          ? "Extracted from label. No application value was provided for comparison."
    ...
    if (!expected) {
      return {
        ...
        confidence: extractedField?.confidence ?? 0,
        explanation: "Extracted from label. No application value was provided for comparison."
      };
    }
    ...
    return {
      ...
      expected,
      extracted: actual,
      status: extractedField?.confidence < 0.55 ? ("warning" as const) : ("fail" as const),
      confidence: extractedField?.confidence ?? 0,
      explanation:
  ...
  const summary =
    !hasApplicationData && overallStatus === "pass"
      ? "Label extraction completed. Add application data to run side-by-side comparison."
      : overallStatus === "pass"
        ? "No blocking issues detected. Human review is still recommended before approval."
        : overallStatus === "review"
          ? "One or more fields need agent review due to uncertainty or formatting concerns."
